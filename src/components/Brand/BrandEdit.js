...
const BrandEdit = () => {
  ...
  useEffect(() => {
    if (formState.data) {
      const product_new = formState.data?.product_commission;
      setValue('commission', (product_new || []).map((item) => {
        return { 
          checked: true,
          value: Number(item.commission),
          product_id: String(item.product_id)
      ...
      // console.log(JSON.stringify(getValues('commission')));
